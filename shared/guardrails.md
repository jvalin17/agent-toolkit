...
#### G-IMPL-7: Ground Truth — No Inference-Based Test Fixtures
Test fixtures must cite their data source. "I read the code" is not a valid source.

**Valid provenance:**
- Query output (`SELECT`, `curl`, API response) — paste the actual output
- User-provided sample data
- Spec/contract that defines format (RFC, OpenAPI schema)
- Generated by test infrastructure (Factory, Faker, builder pattern)
...
**Enforcement (strict mode):** /precommit checks fixture provenance. Block commit if any fixture uses specific-looking values (real dates, IDs, names) with no stated origin.

**If triggered:** "BLOCKED (G-IMPL-7): Test fixture in [file:line] has no provenance citation. Source must be: query output, user sample, spec, or factory. 'I read the code' is not valid."
